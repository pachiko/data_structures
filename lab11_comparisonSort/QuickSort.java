...
public class QuickSort {
    ...
    private static <Item extends Comparable> void partition(
            ...
            for (Item i: unsorted) {
                ...
                if (c < 0) less.enqueue(i);
                else if (c > 0) greater.enqueue(i);
    ...
    public static <Item extends Comparable> Queue<Item> quickSort(Queue<Item> items) {
        ...
        Queue<Item> lt = new Queue<>();
        Queue<Item> eq = new Queue<>();
        Queue<Item> gt = new Queue<>();
        partition(items, getRandomItem(items), lt, eq, gt);
        lt = quickSort(lt);
        gt = quickSort(gt);
        Queue<Item> res = catenate(lt, eq);
        res = catenate(res, gt);
        return res;
    }
}
