...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private boolean debugHeap = false;
    ...
    private void swap(int index1, int index2) {
        ...
        indexMap.put(node1.myItem, index2);
        indexMap.put(node2.myItem, index1);
    }


    /**
     * Returns the index of the node with smaller priority.
     * Precondition: nodes cannot be both null
     */
    ...
    private void swim(int index) {
        ...
        while (parentIdx > 0 && min(parentIdx, index) == index) {
    ...
    private void sink(int index) {
        ...
        while (min(minChildIdx, index) != index) {
            swap(minChildIdx, index);
            index = minChildIdx;
            minChildIdx = min(leftIndex(index), rightIndex(index));
        }
    }
    ...
    /**
     * Returns the Item with the smallest priority value, and removes it from
    ...
    public T removeMin() {
        ...
        contents[size] = null;
        indexMap.remove(min.myItem);
        indexMap.put(getNode(1).myItem, 1);
        size--;
        sink(1);
        return min.item();
    }
    ...
    public void changePriority(T item, double priority) {
        ...
        Node n = contents[idx];
        if (n.myPriority == priority) return;
        boolean swim = priority < n.myPriority;
    ...
    public void testSwim() {
        ...
        if (debugHeap) {
            System.out.println("PQ after changePriority (low):");
