...
public class LinkedListDequeTiming {
    ...
    * */
    private static double timeRemove(int numElements, int numOps, boolean first) {
        ...
        for (int i = 0; i < numElements; i++) {
            if (first) list.addFirst(i);
            else list.addLast(i);
        }

        Stopwatch sw = new Stopwatch();
        for (int i = 0; i < numOps; i++) {
            if(first) list.removeFirst();
    ...
    * */
    private static void timeGet(LinkedListDeque<Integer> list, boolean recursive) {
        int numElements = list.size();

        Stopwatch sw = new Stopwatch();

        int first;
        if (recursive) first = list.getRecursive(0);
        else first = list.get(0);
        double getStart = sw.elapsedTime();

        int last;
        if (recursive) last = list.getRecursive(numElements - 1);
        else last = list.get(numElements - 1);
        double getEnd = sw.elapsedTime() - getStart;

        int midLeft;
        if (recursive) midLeft = list.getRecursive(numElements/2 - 1);
        else midLeft = list.get(numElements/2 - 1);
        double getMidLeft = sw.elapsedTime() - getEnd;

        int midRight;
        if (recursive) midRight = list.getRecursive(numElements/2);
        else midRight = list.get(numElements/2);
    ...
    public static void timeGet() {
        ...
        System.out.println("Timing Get Recursive");
        timeGet(list, true);
    }
}
